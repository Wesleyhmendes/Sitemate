...
export default class UserModel implements IUsersModel {
  ...
  async findAllIssues(): Promise<{ status: string, data: IUsers[] | null }> {
    const infos = await this.userModel.findAll();
    if (infos) {
  ...
  async createIssue(issueData: IUsers): Promise<{ status: string, data: IUsers | null }> {
  ...
  async deleteIssue(issueId: number): Promise<{ status: string, data: number }> {
    const deletedRows = await this.userModel.destroy({ where: { id: issueId } });
    return { status: 'SUCCESSFUL', data: deletedRows };
  }
}
